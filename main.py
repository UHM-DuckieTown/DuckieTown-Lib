...
def runRoadTracking(q, flag,slider):
...
def paho(q, flag, slider):
        p_mqtt.paho_client(q, slider)
...
def main():
        ...
        for func in cameraFunctions:
            p = multiprocessing.Process(target=func, args=(q,flag,slider))
            jobs.append(p)
            p.daemon = True
            p.start()
